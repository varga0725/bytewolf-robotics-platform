...
if importlib.util.find_spec("jsonschema") is None:  # pragma: no cover - environment guard
    # jsonschema is a declared dependency, but the vision research interpreter
    # carries the CV stack instead. Skipping keeps that suite green while the
    # same tests run wherever the dependency is installed.
    raise unittest.SkipTest("jsonschema is required to validate the shared Vision schemas")

from brain.vision.canonical import (  # noqa: E402 - imported after the environment guard
