...
def _detect(model, image_path: Path, source: str, max_age_s: float) -> dict[str, object]:
        ...
        for name, image_text, output_text in args.camera:
            ...
            try:
                ...
                if seen.get(name) == stamp:
                    continue
                _atomic_json(output_path, _detect(model, image_path, f"yolo11n {name}", args.max_age_s))
                seen[name] = stamp
